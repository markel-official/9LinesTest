...
/*
I know it's not the right way to do it. But there's some error with gulp/webpack "dest" which
I can't resolve yet. But I'm willing to learn!
*/
gulp.task('js-preloader', () => {
	return gulp.src('./src/js/preloader.js')
		// .pipe($.plumber({
		// 	errorHandler,
		// }))
		// .pipe($.webpackStream(webpackConfig.module))
		// .pipe(gulp.dest(webpackConfig.output.path));
		.pipe(gulp.dest('build/js'));
});
...
gulp.task('watch', () => {
	...
	gulp.watch('src/js/**/*.js', gulp.series('js', 'js-preloader'));
});
